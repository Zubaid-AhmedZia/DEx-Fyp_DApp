...
contract('Dex', (accounts) => {
  ...
  it('should NOT create limit order if token balance too low', async () => {
    ...
    await expectRevert(
      dex.createLimitOrder(
        REP,
        web3.utils.toWei('100'), //sell 100 tokens at 10 price
        10,
        SIDE.SELL,
        {from: trader1}
      ),
  ...
  it('should NOT create limit order if dai balance too low', async () => {
    ...
    await expectRevert(
      dex.createLimitOrder(
        REP,
        web3.utils.toWei('10'), //buy 10 REP at 10price
        10,
        SIDE.BUY,
        {from: trader1}
      ),
  ...
  it('should create market order & match', async () => {
    ...
    assert(balances[0].toString() === web3.utils.toWei('50')); //buys REP at 10 dai per token so left with 50 dai. 
    assert(balances[1].toString() === web3.utils.toWei('5')); //now have 5 REP
    assert(balances[2].toString() === web3.utils.toWei('50')); // trader2 have 50 as bcz it sold 5 rep for 10 dai per token
    assert(balances[3].toString() === web3.utils.toWei('95')); //100 -5 REP = 95 REP remaining
  });
...
//buying all without any dai in trader2
    await expectRevert(
      dex.createMarketOrder(
        REP,
        web3.utils.toWei('100'),
